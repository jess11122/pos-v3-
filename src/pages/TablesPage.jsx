...
export default function TablesPage() {
  ...
  const loadTables = useCallback(async () => {
    ...
    for (const o of (orders || [])) {
      if (!grouped[o.table_number]) {
        grouped[o.table_number] = { tableNumber: o.table_number, orders: [], staffName: o.staff_name, staffColour: o.staff_colour, openedAt: o.created_at, barPending: false, kitchenPending: false, ready: false }
      }
    ...
    for (const r of (routed || [])) {
      ...
      for (const t of Object.values(grouped)) {
        if (t.orders.some(o => o.id === r.order_id)) {
          if (r.routed_to === 'bar' && ['pending', 'making'].includes(r.status)) t.barPending = true
          if (r.routed_to === 'kitchen' && ['pending', 'making'].includes(r.status)) t.kitchenPending = true
          if (r.routed_to === 'kitchen' && r.status === 'ready') t.ready = true
        }
      }
    }
  ...
  const openTable = async (t) => {
    ...
    setTip('')
    setCustomTip('')
    const ids = t.orders.map(o => o.id)
    const { data } = await supabase.from('order_items_routed').select('*').in('order_id', ids)
    setTableOrders(data || [])
  }
  ...
  const handlePayment = async (method) => {
    ...
    try {
      ...
      await supabase.from('orders').update({ status: 'complete', tab_closed: true, completed_at: new Date().toISOString(), payment_method: method }).in('id', orderIds)
  ...
  const allItems = selectedTable ? selectedTable.orders.flatMap(o => (o.items || []).map(i => ({ ...i, note: o.note, allergens: o.allergens }))) : []
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="font-barlow text-zinc-500 text-lg">No open tabs</p>
          </div>
        ) : (
            ...
            {selectedTable.orders.some(o => o.allergens?.length > 0) && (
              <div className="bg-red-900/30 border border-red-800 rounded-xl px-4 py-3">
                <p className="font-barlow text-red-400 text-sm">
                  ⚠ Allergens noted: {[...new Set(selectedTable.orders.flatMap(o => o.allergens || []))].join(', ')}
                </p>
              </div>
            )}
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2 flex-wrap">
                {[2, 5, 10, 20].map(amt => (
                  <button
                    key={amt}
                    onClick={() => { setTip(String(amt)); setCustomTip('') }}
...
function TableCard({ table, onClick }) {
  ...
  return (
    <button onClick={onClick} className="bg-zinc-800 hover:bg-zinc-750 border border-zinc-700 hover:border-zinc-500 rounded-2xl p-5 text-left transition-all active:scale-98 w-full">
      ...
      <div className="flex items-center gap-2 mb-1">
        <div className="w-2 h-2 rounded-full" style={{ backgroundColor: table.staffColour }} />
